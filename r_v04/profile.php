    ...
    if ($result = $conn->query($query)) {
      ...
      if ($result = $conn->query($query)) {
          ...
          while ($row = $result->fetch_assoc()) {
            ...
            if($_SESSION["userName"] == $row["currentUser"] && $row["current_request"] == 1){
      ...
      // display past friend requests
            echo '<div class="container-fluid">';
            echo '<div class="container-sm posts shadow p-3 mb-5 bg-white rounded">';
            echo '<div class="container title"><h3>previous friend requests </h3></div>';
            ...
            if ($result = $conn->query($query)) {
                ...
                while ($row = $result->fetch_assoc()) {
                  ...
                  if($_SESSION["userName"] == $row["currentUser"] && $row["completed_requests"] == 1){
                    echo '<div class="container content">';
                    echo '<a href="user_profile.php?usrName='.$row["friend_request_name"].'">'; echo $row["friend_request_name"]; echo '</a>';
                    echo '<br>';
                    echo '<br>';
                    echo '</div>';
                 }
